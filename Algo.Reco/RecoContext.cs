...
namespace Algo
{
    public class RecoContext
    {
        ...
        public static double DNorme2(User user1, User user2 )
        {
            double distPower2 = 0.0;
            int match = 0;
            foreach( var rating in user1.Ratings )
            {
                int rate;
                if(user2.Ratings.TryGetValue( rating.Key, out rate ) )
                {
                    distPower2 += Math.Pow( rating.Value - rate, 2 );
                    match++;
                }
            }
            return match == 0 ? double.MaxValue : Math.Sqrt( distPower2 );
        }
    }
}
